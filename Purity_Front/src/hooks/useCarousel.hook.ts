        ...
        // Function to get new page width on resize event
            const handleResize = () => {
                ...
                if (elementRef.current!.children.length >= 3) {
                    ...
                    const calculatedChildWidth = Number(window.getComputedStyle(elementRef.current!.children[0]).getPropertyValue('width').slice(0, -2));

                    // Get width of an element
                    const calculatedElementWidth = Number(window.getComputedStyle(elementRef.current!.parentElement as Element).getPropertyValue('width').slice(0, -2));
                    ...
                    const calculatedElementGap = (calculatedElementWidth - calculatedChildWidth * calculatedChildrenFit) / calculatedChildrenFit;
        ...
        // Setup reducer
            ...
            function reducerPageCycle(state: PageCycle, action: PageCycleAction): PageCycle {
                ...
                // Pass requested page of define it if it havent been defined yet
                    const requestedPage = state.requested ? state.requested : action.requestedPage;
